      ...
      <div className="p-1 rounded-[1.5rem] bg-white/[0.02] border border-dashed border-white/10">
        ...
        <div className="grid grid-cols-1 gap-2 p-1.5">
          {accounts.map((account, index) => (
            <motion.button
              ...
              className="flex items-center gap-3 p-2 rounded-2xl bg-white/[0.03] border border-white/5 text-left transition-colors group hover:border-emerald-500/30"
            >
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  {account.badge && (
                    <span className="px-1.5 py-0.5 rounded-full bg-emerald-500/10 text-[7px] font-black uppercase tracking-wider text-emerald-500 border border-emerald-500/20">
                      {account.badge}
                    </span>
                  )}
                </div>
        ...
        <div className="px-4 pb-2 pt-1 text-center">
          <p className="text-[9px] font-bold text-slate-600 uppercase tracking-widest italic">
            Password: <span className="text-emerald-500/70 ml-1 select-all">{accounts[0]?.password}</span>
          </p>
        </div>
      </div>
    </div>
