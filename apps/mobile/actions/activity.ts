...
export async function fetchRecentlyViewed(params: ActivityParams = {}): Promise<Product[]> {
  const deviceId = await getDeviceId();
...
export async function fetchForYou(params: ActivityParams = {}): Promise<Product[]> {
  const deviceId = await getDeviceId();
