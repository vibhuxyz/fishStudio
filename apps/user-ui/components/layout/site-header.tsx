...
export function SiteHeader({ onLoginClick, onCartClick }: SiteHeaderProps) {
  ...
  const selectedAddress = hydrated ? getSelectedAddress() : null;
  const addressLine = selectedAddress
    ? `${selectedAddress.label} - ${selectedAddress.area}, ${selectedAddress.city}`
    : "Select location";
  const addressShort =
    addressLine.length > 28 ? addressLine.slice(0, 28) + "…" : addressLine;
