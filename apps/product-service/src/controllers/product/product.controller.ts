...
//////////////////////////////////////////////////////////////////////////////
// HOMEPAGE SECTIONS + USER ACTIVITY (recently viewed / for you)
//////////////////////////////////////////////////////////////////////////////
...
export const getHomepageSections = async (
  ...
  try {
    ...
    const baseWhere = { adminId: { not: null }, isDeleted: false };

    const [freshArrivals, bestSellers, combos, seasonal] = await Promise.all([
      // Fresh Arrivals: tagged fresh-today/fresh-arrival, fallback to newest.
      fetchSectionProducts({
        where: {
          ...baseWhere,
          OR: [
            { tags: { has: "fresh-today" } },
            { tags: { has: "fresh-arrival" } },
          ],
        },
      ...
      // Combos: a "Combos" category OR products tagged "combo".
      fetchSectionProducts({
        where: {
          ...baseWhere,
          OR: [
      ...
      // Seasonal Specials: a "Seasonal Specials" category OR tagged "seasonal".
      fetchSectionProducts({
        where: {
          ...baseWhere,
          OR: [
    ...
    // Fresh Arrivals falls back to newest products when nothing is tagged yet.
    const freshArrivalsResolved = freshArrivals.length
      ? freshArrivals
    ...
    const sections = [
      { key: "fresh-arrivals", title: "Fresh Arrivals", products: freshArrivalsResolved },
      { key: "best-sellers", title: "Best Sellers", products: bestSellers },
      { key: "combos", title: "Combos", products: combos },
      { key: "seasonal", title: "Seasonal Specials", products: seasonal },
